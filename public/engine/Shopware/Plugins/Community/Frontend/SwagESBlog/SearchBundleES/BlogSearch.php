...
class BlogSearch implements ProductSearchInterface
{
    ...
    {
        error_log('---===>>> HERE !!!');
        error_log('---===>>> ' . $criteria->hasCondition('search'));
        ...
        error_log('---END-RESULT---===>>> ' . json_encode($result) . '<<<');
        return $result;
    }
    ...
    private function searchBlog(Criteria $criteria, Struct\ProductContextInterface $context)
    {
        ...
        error_log('---QUERY---===>>> ' . json_encode($query->toArray()) . '<<<');
        ...
        error_log('---RESULT---===>>> ' . json_encode($raw) . '<<<');
    ...
    {
        error_log('---TERM---===>>> ' . json_encode($condition->getTerm()) . '<<<');
